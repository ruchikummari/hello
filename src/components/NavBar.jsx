...
const NavBar = () => {
    ...
    return (
        <div className="fixed top-0 left-0 w-full z-60 bg-white shadow-md flex items-center justify-between px-3 md:px-6 lg:px-25 pt-5">

            <Link to='/'><img src={logo} className="w-35" alt="logo" /></Link>

            {/* Desktop Navigation */}
            <ul className="sm:hidden md:flex gap-5 text-sm text-grey-700">
            ...
            <div className="flex items-center gap-2 md:gap-4">
                ...
                <img
                    ...
                    src={menu}
                    className="w-5 md:hidden cursor-pointer hover:opacity-80 transition-opacity z-[70]"
                    alt="menu"
                />
            </div>

            {/* Mobile Menu */}
            <div className={`fixed top-0 right-0 bottom-0 w-full md:w-80 bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${visible ? 'translate-x-0' : 'translate-x-full'} md:hidden z-[60]`}>
                <div className="flex flex-col text-gray-600 h-full">
                    <div onClick={() => setVisible(false)} className="flex items-center gap-4 p-4 border-b cursor-pointer">
                        <img className="h-4 " src={dropdown} alt="back" />
                        <p>Back</p>
                    </div>
